...
void validate_tiled(
    ...
    memory_layout_t mem_layout)
{
    // Use double-precision scaling factor for the validation.
...
// A Sylvester equation AX + sgn * XB = C has a unique solution for X if the
// spectra of A and sgn * B are disjoint.
int validate_spectra(double sgn,
    int m, double *lambda_A, int *lambda_type_A,
    int n, double *lambda_B, int *lambda_type_B)
{
    // The smart way would be to sort the eigenvalues by magnitude and then do a
    // merge-sort-like comparison. Let's just be stupid.
